...
if (message.author.bot) return;
        ...
        if(!log[message.guild.id]) log[message.guild.id] = {
          ...
          if(log[message.guild.id].onoff === 'Off') return [message.channel.send(`**The log Is __ً‌گژً‌گچ__ !**`), log[message.guild.id].onoff = 'On']
          if(log[message.guild.id].onoff === 'On') return [message.channel.send(`**The log Is __ً‌گژً‌گ…ً‌گ…__ !**`), log[message.guild.id].onoff = 'Off']
          fs.writeFile("./log.json", JSON.stringify(log), (err) => {
            if (err) console.error(err)
            .catch(err => {
              console.error(err);
    ...
    oldMember.guild.fetchAuditLogs().then(logs => {
        ...
        if(oldMember.nickname !== newMember.nickname) {
            if(oldMember.nickname === null) {
                var oldNM = '`ط§ط³ظ…ظ‡ ط§ظ„ط§طµظ„ظٹ`';
            }else {
                var oldNM = oldMember.nickname;
            }
            if(newMember.nickname === null) {
                var newNM = '`ط§ط³ظ…ظ‡ ط§ظ„ط§طµظ„ظٹ`';
            }else {
